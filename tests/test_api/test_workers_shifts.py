...
def test_post_workers_shifts_day_limit(app, test_client, regular_worker_shift, regular_worker, regular_shift):
    url = '/api/v1/workers-shifts/'
...
def test_post_workers_shifts(app, test_client, regular_worker, regular_shift):
    url = '/api/v1/workers-shifts/'
    ...
    worker_shift = WorkerShift.query.one()
    assert response_util.parse_data(response) == {
        'id': str(worker_shift.id),
        'type': 'workers_shifts',
        'attributes': {
        ...
        'relationships': {
            'shift': {
                'data': {
                    'id': str(regular_shift.id),
                    'type': 'shifts'
                },
                'links': {
                    'related': f'/api/v1/shifts/{str(regular_shift.id)}',
                    'self': f'/api/v1/workers-shifts/{str(worker_shift.id)}/relationships/shift'
                }
            },
            'worker': {
                'data': {
                    'id': str(regular_worker.id),
                    'type': 'workers'
                },
                'links': {
                    'related': f'/api/v1/workers/{str(regular_worker.id)}',
                    'self': f'/api/v1/workers-shifts/{str(worker_shift.id)}/relationships/worker'
                }
            }
        }
